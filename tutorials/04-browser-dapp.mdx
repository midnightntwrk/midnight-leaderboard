...
title: "Browser DApp"
description: "Build a React frontend that connects to the Lace wallet, reads leaderboard state from the Midnight indexer, and submits scores on-chain with privacy modes."
sidebar_position: 4
...
Ownership verification calls `verifyOwnership` on the API and tracks verified entries in local React state. If the caller is not the owner, the contract's `assert` fails and the network rejects the transaction.
