...
title: "Browser DApp"
description: "Build a React frontend with Lace wallet integration, indexer reads, and on-chain score submission."
sidebar_position: 4
...
In this section, you will build a React frontend that connects to the Lace wallet, reads leaderboard data from the indexer, submits scores on-chain, and lets players prove ownership of their entries.
...
The TypeScript configuration needs `"types": ["vite/client"]` so that `import.meta.env` references compile correctly. The `"bundler"` module resolution is required for Midnight SDK subpath imports.
...
Create the local development environment file. `VITE_PROOF_SERVER_URL` points to the local Docker proof server. Leave `VITE_DEFAULT_CONTRACT` empty until you deploy a contract in a later step.
...
Create the HTML shell that Vite uses as the application entry point. The `src/main.tsx` script is loaded as an ES module.
...
The `walletProvider.balanceTx` method is the bridge between the DApp and the user's wallet. The DApp builds an unbalanced transaction with the contract circuit proof, sends it to Lace for balancing (adding DUST fees and Zswap proofs), and gets back a finalized transaction ready to submit.

`setNetworkId()` must be called before initializing any providers.
...
Ownership verification calls `verifyOwnership` on the API and tracks verified entries in local React state. If the caller is not the owner, the contract's `assert` fails and the transaction is rejected.
...
> **Note:** Deploying a contract requires tDUST in your wallet. If you see a transaction submission error, go to Lace, open the Tokens tab, and click **Generate tDUST**.
