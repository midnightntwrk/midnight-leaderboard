...
`scores` is the public ledger readable from the indexer. `nextId` auto-increments to assign unique entry IDs. The `witness` declaration tells the compiler that the TypeScript host provides `getCustomName` at runtime.
...
The caller proves that `hash(theirPublicKey) == entry.ownerHash` without revealing the public key itself. The prover generates the proof locally. You can use it to claim a prize, verify identity, or present to a badge system. It does not write anything to the ledger.
